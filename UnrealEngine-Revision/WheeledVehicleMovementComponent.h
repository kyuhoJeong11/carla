...
public:
	...
	/** If true, the wheel will use sweep for collisions, this will give a more reallistic behaviour but also computationally more expensive. If disabled, the car will use single raycast. */
	UPROPERTY(EditAnywhere, Category = VehicleSetup)
...
protected:
	...
	FVehicleInputRate RPMInputRate;
	...
	*/
	...
	void ServerUpdateState(float InRPMInput, float InSteeringInput, float InThrottleInput, float InBrakeInput, float InHandbrakeInput, int32 CurrentGear);
