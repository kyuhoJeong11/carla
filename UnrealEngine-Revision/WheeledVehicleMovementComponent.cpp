	...
	: Super(ObjectInitializer)
{
	...
	RPMInputRate.RiseRate = 6.0f;
	RPMInputRate.FallRate = 10.0f;
...
void UWheeledVehicleMovementComponent::UpdateState(float DeltaTime)
{
	...
	if (Controller && Controller->IsLocalController())
	{
		...
		ServerUpdateState(RPMInput, SteeringInput, ThrottleInput, BrakeInput, HandbrakeInput, GetCurrentGear());
	...
	else
	{
		...
		RPMInput = ReplicatedState.RPMInput;
...
// 매 Tick마다 호출됨.
bool UWheeledVehicleMovementComponent::ServerUpdateState_Validate(float InRPMInput, float InSteeringInput, float InThrottleInput, float InBrakeInput, float InHandbrakeInput, int32 InCurrentGear)
{
	//UE_LOG(LogTemp, Warning, TEXT("ServerUpdateState_Validate"));

	return true;
}

// 매 Tick마다 호출됨.
void UWheeledVehicleMovementComponent::ServerUpdateState_Implementation(float InRPMInput, float InSteeringInput, float InThrottleInput, float InBrakeInput, float InHandbrakeInput, int32 InCurrentGear)
{
	// ThrottleInput 주석 처리 시 정상 실행.
	RPMInput = InRPMInput;
	...
	// ThrottleInput을 주석 처리 해봤지만 문제 없이 정상적으로 작동됨. 어디서 쓰이는 것인지?
	//UE_LOG(LogTemp, Warning, TEXT("ServerUpdateState_implementation"));
	ReplicatedState.RPMInput = InRPMInput;
...
/// @endcond
// Calc 메서드들은 매 Tick마다 호출 됨.
float UWheeledVehicleMovementComponent::CalcRPMInput()
{
	//UE_LOG(LogTemp, Warning, TEXT("Calc RPM"));
...
{
	//UE_LOG(LogTemp, Warning, TEXT("Calc Steering"));
...
void UWheeledVehicleMovementComponent::SetEngineRotationSpeed(float Speed1, float Speed2, float Speed3, float Speed4) const
{
	...
	PVehicle->mWheelsDynData.setWheelRotationSpeed(2, Speed3);
	PVehicle->mWheelsDynData.setWheelRotationSpeed(3, Speed4);
}
...
float UWheeledVehicleMovementComponent::GetEngineRotationSpeed() const
{
	...
	{
		UE_LOG(LogVehicles, Warning, TEXT("PVehicle && WheelSetups"));
