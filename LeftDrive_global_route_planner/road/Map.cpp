...
namespace road {
  ...
  boost::optional<Waypoint> Map::GetClosestWaypointOnRoad(
    ...
    // 10/08 -> 10/12(화) 
    // 1. segment 변경해보기 및 result_start.lane_id 조정해보기
    // 2. junction 관련 함수 찾아서 수정해보기.
    // 3. 이후 변화가 없을 시 global route planner 파일 수정 시도해보기.
    // 4. MapBuilder 파일 확인해보기.(MapBuilder의 문제일 가능성은 낮음.)
	  ...
	  for (const auto &pair : _data.GetRoads()) {
		  ...
		  ForEachDrivableLane(road, [&](auto &&waypoint) {
			  for (auto &&predecessor : GetPredecessors(waypoint)) {
				  result.push_back({ waypoint, predecessor });
			  }
		  });
	  }
	  return result;
