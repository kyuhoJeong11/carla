...
namespace client {
  ...
  std::vector<std::pair<SharedPtr<Waypoint>, SharedPtr<Waypoint>>> Map::GetJunctionWaypoints(
    ...
    for (auto &waypoint_pair : junction_waypoints) {
      result.emplace_back(
      std::make_pair(SharedPtr<Waypoint>(new Waypoint(shared_from_this(), waypoint_pair.second)),
      SharedPtr<Waypoint>(new Waypoint(shared_from_this(), waypoint_pair.first))));
    }
    return result;
  }
