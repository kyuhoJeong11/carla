...
namespace road {
  ...
  static std::pair<double, double> ComputeTotalLaneWidth(
    ...
    for (const auto &lane : container) {
      ...
      if (lane.first != lane_id) {
        ...
        current_dist *= -1.5;
        dist += negative_lane_id ? current_dist : -current_dist;
        tangent += (negative_lane_id ? current_tang : -current_tang) * -1.5;
        break;
      }
    }
    return std::make_pair(dist, tangent);
  }
  // 05/12 함수 오버로딩을 통해 ComputeTotalLaneWidth를 이용한 lane_width 수정해보기.
  ...
  */
  ...
  static std::pair<double, double> ComputeTotalLaneWidth(
    ...
    for (const auto &lane : container) {
      ...
      if (lane.first != lane_id) {
        ...
        current_dist *= (-0.5 + cnt);
        dist += negative_lane_id ? current_dist : -current_dist;
        tangent += (negative_lane_id ? current_tang : -current_tang) * (-0.5 + cnt);
        break;
      }
    }
    return std::make_pair(dist, tangent);
  ...
  geom::Transform Lane::ComputeTransform(const double s) const {
    ...
    if((GetId() == 4 || GetId() == -4) && GetType() == Lane::LaneType::Sidewalk) flag = false;
    if(lane_cnt == 0 && GetId() < -4) lane_cnt = GetId() + 4;
    ...
    if (GetId() < 0) {
      ...
      if(flag){
        const auto computed_width =
            ComputeTotalLaneWidth(side_lanes, s, GetId(), lane_cnt);
